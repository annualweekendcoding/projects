...
camera = picamera.PiCamera(resolution=(width, height),framerate=80)
try:
    while True:
        ...
        with picamera.array.PiRGBArray(camera) as stream:
            ...
            camera.capture(stream, format='bgr', use_video_port=use_video_port)
            img = stream.array

        image = detect(img, faceCascade)

#        cv.imshow("camera", image)
        k = chr(cv.waitKey(10) & 255)
        if k == 'q':
            break
        elif k == '+':
            if camera.brightness <= 95:
                camera.brightness += 5
        elif k == '-':
            if camera.brightness > 5:
                camera.brightness -= 5
        elif k == '*':
            if camera.contrast <=95:
                camera.contrast += 5
        elif k == '_':
            if camera.contrast >= -95:
                camera.contrast -= 5
