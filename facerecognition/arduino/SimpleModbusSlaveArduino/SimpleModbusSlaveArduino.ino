...
Servo servo;
unsigned long earliestSetTime = 0;
int lastValue = 512;
////////////////////////////////////////////////////////////

void setup()
{
  holdingRegs[PWM_VAL] = lastValue;
...
void loop()
{
  ...
  int regval = holdingRegs[PWM_VAL];
  int val = map(regval, 0, 1024, 10, 170);
  if (regval != lastValue && millis() > earliestSetTime)
  {
    servo.write(val);
    lastValue = regval;
    
    // Dem Servo Zeit lassen zum einstellen
    // (wobei 400ms wahrscheinlich mehr als ausreichend sind)
    earliestSetTime = millis() + 400;
  }
