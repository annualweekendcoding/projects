...
#ifdef DS18X20_LCDSUPPORT
#include "textlcd4.h"
void DS18X20_lcd_put_temp(const uint8_t subzero, 
	const uint8_t cel, 	const uint8_t cel_frac_bits)
{
	char buffer[6];
	int i;
	
	lcd_put_char((subzero)?'-':'+');
	lcd_printInt((int)cel);
	lcd_put_char('.');
	itoa(cel_frac_bits*DS18X20_FRACCONV,buffer,10);
	for (i=0;i<4-strlen(buffer);i++) lcd_put_char('0');
	lcd_print(buffer);
	lcd_print("ßC");
}
#endif
...
void DS18X20_show_id_uart( uint8_t *id, size_t n )
{
  ...
  {
	  if ( i == 0 ) lcd_print( "FC:" );
		else if ( i == n-1 ) lcd_print( "CRC:" );
		if ( i == 1 ) lcd_print( "SN: " );
		lcd_printHex(id[i]);
		lcd_put_char(' ');
		if ( i == 0 ) 
    {
			if ( id[0] == DS18S20_ID ) lcd_print ("(18S)");
			else if ( id[0] == DS18B20_ID ) lcd_print ("(18B)");
			else lcd_print ("( ? )");
		}
	}
...
uint8_t DS18X20_read_meas_all_verbose( void )
{
		...
		if( diff == OW_DATA_ERR ) 
    {
		...
		if( id[0] == DS18B20_ID || id[0] == DS18S20_ID ) {	 // temperature sensor
			...
			lcd_print(" T_raw=");
			lcd_printHex((uint8_t)(meas>>8));
			lcd_printHex((uint8_t)meas);
			lcd_put_char(' ');
			...
			DS18X20_lcd_put_temp(subzero, cel, cel_frac_bits);
...
uint8_t DS18X20_start_meas( uint8_t with_power_extern, uint8_t id[])
{
    ...
    return DS18X20_OK;
		...
		#ifdef DS18X20_VERBOSE
		lcd_print( "DS18X20_start_meas: Short Circuit !\r" );
		#endif
	...
	uint8_t th, uint8_t tl, uint8_t conf)
{
	...
	if( ow_input_pin_state() ) { // only send if bus is "idle" = high
		...
		#ifdef DS18X20_VERBOSE
		lcd_print( "DS18X20_write_scratchpad: Short Circuit !\r" );
		#endif
...
uint8_t DS18X20_read_scratchpad( uint8_t id[], uint8_t sp[] )
{
	...
	if( ow_input_pin_state() ) { // only send if bus is "idle" = high
		...
		#ifdef DS18X20_VERBOSE
		lcd_print( "DS18X20_read_scratchpad: Short Circuit !\r" );
		#endif
		return DS18X20_ERROR;
	}
}
	...
	uint8_t id[] )
{
	...
	if( ow_input_pin_state() ) { // only send if bus is "idle" = high
		...
		#ifdef DS18X20_VERBOSE
		lcd_print( "DS18X20_copy_scratchpad: Short Circuit !\r" );
		#endif
		return DS18X20_START_FAIL;
	}
}
...
uint8_t DS18X20_recall_E2( uint8_t id[] )
{
	...
	if( ow_input_pin_state() ) { // only send if bus is "idle" = high
		...
		#ifdef DS18X20_VERBOSE
		lcd_print( "DS18X20_recall_E2: Short Circuit !\r" );
		#endif
		return DS18X20_ERROR;
	}
}
...
#if DS18X20_MAXSENSORS>0 

uint8_t DS18X20_SensorIDs[DS18X20_MAXSENSORS][OW_ROMCODE_SIZE];

uint8_t DS18X20_search_sensors(void)
{
	uint8_t i;
	...
	for( diff = OW_SEARCH_FIRST; 
		diff != OW_LAST_DEVICE && nSensors < DS18X20_MAXSENSORS ; )
	{
		...
		for (i=0;i<OW_ROMCODE_SIZE;i++)
			DS18X20_SensorIDs[nSensors][i]=id[i];
