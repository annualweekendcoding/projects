...
//default preamble (alternating 1s and 0s)
...
#define SS_ASSERT() RFM12_PORT_SS &= ~(1<<RFM12_BIT_SS)
#define SS_RELEASE() RFM12_PORT_SS |= (1<<RFM12_BIT_SS)
...
#if RFM12_DEBUGLEVEL>0
volatile uint8_t rx_int_count =0;
volatile uint8_t tx_int_count =0;
#endif
...
struct rf_tx_buffer_t rf_tx_buffer;
volatile int rf_tx_lasttime;
#endif

#if RFM12_RX_BUFFER_SIZE>0
//buffer and status for the message to be received
struct rf_rx_buffer_t rf_rx_buffer;
// save the time of last receive in ms
volatile int rf_rx_lasttime;
volatile uint8_t rf_rx_last_error;
#endif
...
static uint8_t spi_data(uint8_t c)
{
	...
	for(x=0;x<8;x++)
  {
		if(c & 0x80)
    {
			RFM12_PORT_MOSI |= (1<<RFM12_BIT_MOSI);
		}
    else
    {
			RFM12_PORT_MOSI &= ~(1<<RFM12_BIT_MOSI);	
		}
		RFM12_PORT_SCK |= (1<<RFM12_BIT_SCK);
		d<<=1;
		if(RFM12_PIN_MISO & (1<<RFM12_BIT_MISO))
    {
			d|=1;
		}
		RFM12_PORT_SCK &= ~(1<<RFM12_BIT_SCK);
...
{
	SS_ASSERT();
#if defined RFM12_SPI_SOFTWARE
	spi_data(d >> 8);
	spi_data(d & 0xff);
#elif defined RFM12_SPI_USI
	USIDR = d >> 8;
  USISR = (1<<USIOIF);
  do
  {
	...
	if(status & (RFM12_STATUS_FFIT>>8))
	{
		...
		if (rfm12_mode == MODE_TX)
		{
      ...
      else
			{
				...
				rf_tx_buffer.status = RFM12_STATUS_FREE;
      ...
      rf_rx_lasttime = time_ms;
      ...
      #if RFM12_DEBUGLEVEL >= 2
  			debug_write_char('R');
  			//debug_write_char(rfm12_read_fifo_inline());
      #endif
			 ...
			 */
			if(rf_rx_buffer.status == RFM12_STATUS_IDLE)
			{
				//read first byte, which is the length byte
				uint8_t length = rfm12_read_fifo_inline();

        #ifndef RFM12_RAW_MODE				
				rf_rx_buffer.bytecount = 1;
				rf_rx_buffer.num_bytes = length + RFM12_HEADER_SIZE;
				#endif

				if(rf_rx_buffer.rf_buffer_in->status == RFM12_STATUS_FREE)
        {
					//the current receive buffer is empty, so we start receiving
					rf_rx_buffer.status = RFM12_STATUS_RECEIVING;
          #ifndef RFM12_RAW_MODE				
          rf_rx_buffer.rf_buffer_in->len = length;
          #else
          // im RAW-Mode wird das erste Byte in den Puffer gelegt
          // in len werden die Bytes gez�hlt
          rf_rx_buffer.rf_buffer_in->len = 1;
          rf_rx_buffer.rf_buffer_in->buffer[0] = length;
          #endif
				}
        else
        {
          // if the buffer is not free we reset the receiver
          rf_rx_last_error = RFM12_RX_BUFFER_NOT_FREE; 
					//reset fifo
					rfm12_data_inline(RFM12_CMD_FIFORESET>>8, CLEAR_FIFO_INLINE);
					rfm12_data_inline(RFM12_CMD_FIFORESET>>8, ACCEPT_DATA_INLINE);	
				}
			...
			else if(rf_rx_buffer.status == RFM12_STATUS_RECEIVING)
			{
        #ifndef RFM12_RAW_MODE				
				//check if transmission is complete
				if(rf_rx_buffer.bytecount < rf_rx_buffer.num_bytes)
				{
					uint8_t data = rfm12_read_fifo_inline();
					
					//put next byte into buffer, if there is enough space
					if(rf_rx_buffer.bytecount < (RFM12_RX_BUFFER_SIZE + RFM12_HEADER_SIZE))
					{
					...
					//check crc of header
					if((rf_rx_buffer.bytecount == (RFM12_HEADER_SIZE-1)) && (data != crc8(&rf_rx_buffer.rf_buffer_in->len,3)) )
					{
						//invalid crc, reset transmission
						rf_rx_buffer.status = RFM12_STATUS_IDLE;
					
						//reset fifo
						rfm12_data_inline(RFM12_CMD_FIFORESET>>8, CLEAR_FIFO_INLINE);
						rfm12_data_inline(RFM12_CMD_FIFORESET>>8, ACCEPT_DATA_INLINE);

            rf_rx_last_error = RFM12_RX_CRC_ERROR_HEADER; 
					}
				...
				else
				{
					...
					//check crc of data
					if(rf_rx_buffer.rf_buffer_in->crc_data == crc8(rf_rx_buffer.rf_buffer_in->buffer,rf_rx_buffer.rf_buffer_in->len))
          {
  					//indicate that the Buffer is ready to be used
	  				rf_rx_buffer.rf_buffer_in->status = RFM12_STATUS_COMPLETE;
          ...
          {
            rf_rx_last_error = RFM12_RX_CRC_ERROR_DATA; 
					}
          //reset fifo
					rfm12_data_inline(RFM12_CMD_FIFORESET>>8, CLEAR_FIFO_INLINE);
					rfm12_data_inline(RFM12_CMD_FIFORESET>>8, ACCEPT_DATA_INLINE);
        ...
        #else
          // Im RAW-Mode hier nur das n�chste Byte speichern
					uint8_t data = rfm12_read_fifo_inline();
					
					//put next byte into buffer, if there is enough space
					if(rf_rx_buffer.rf_buffer_in->len < RFM12_RX_BUFFER_SIZE)
					{
						rf_rx_buffer.rf_buffer_in->buffer[rf_rx_buffer.rf_buffer_in->len++] = data;
					}
          else
          {
            rf_rx_last_error = RFM12_RX_BUFFER_FULL;
            // Vollgelaufen, dann n�chster Puffer 
  					rf_rx_buffer.status = RFM12_STATUS_IDLE;
  					//indicate that the Buffer is ready to be used
	  				rf_rx_buffer.rf_buffer_in->status = RFM12_STATUS_COMPLETE;
  					//switch to other buffer
	  				uint8_t num = rf_rx_buffer.buffer_in_num;
		  			num = (num+1) % 2;
			  		rf_rx_buffer.buffer_in_num = num;
					  rf_rx_buffer.rf_buffer_in = &rf_rx_buffer.rf_buffers[num];

            // neuen Puffer gleich anfangen
  					rf_rx_buffer.status = RFM12_STATUS_RECEIVING;
            // im RAW-Mode wird das erste Byte in den Puffer gelegt
            // in len werden die Bytes gez�hlt
            rf_rx_buffer.rf_buffer_in->len = 1;
            rf_rx_buffer.rf_buffer_in->buffer[0] = data;
...
{
	uint16_t status;
	static uint16_t channel_free_count = 16;
  	...
  	if (oldmode != rfm12_mode)
  	{
  		...
  		switch (rfm12_mode)
  		{
  			...
  			default:
  				debug_write_char ('x');
          debug_printHex(rfm12_mode);
  		}
  	}
    static uint8_t old_rx_int_count=0;
    if (old_rx_int_count!=rx_int_count)
    {
      old_rx_int_count=rx_int_count;
      debug_write_str("RX");
      debug_printHex(old_rx_int_count);
    }
    static uint8_t old_tx_int_count=0;
    if (old_tx_int_count!=tx_int_count)
    {
      old_tx_int_count=tx_int_count;
      debug_write_str("TX");
      debug_printHex(old_tx_int_count);
    }
    if (rf_rx_last_error)
    {
      debug_write_str("Receive Error:");
      debug_printHex(rf_rx_last_error);
      rf_rx_last_error=0;
      debug_write_endl;
    }
  #endif

  // check timeout of transmitter (ms)
  // TODO: Zeit aus Baudrate und Pufferl�nge berechnen 
  if ((time_ms-rf_tx_lasttime)>100 && rfm12_mode == MODE_TX)
  {
    // Bei Timeout den Sender abschalten
		//we are now in rx mode again
		rfm12_mode = MODE_RX;
		...
		//init receiver fifo
		rfm12_data(RFM12_CMD_FIFORESET | CLEAR_FIFO);
		rfm12_data(RFM12_CMD_FIFORESET | ACCEPT_DATA);
  	RFM12_INT_ON();
    debug_write_str("Transmit Timeout\r\n");
    if(rf_tx_buffer.status == RFM12_STATUS_OCCUPIED)
		{
      #if RFM12_DEBUGLEVEL > 0
        if (rf_tx_buffer.bytecount>0)
        {
          debug_write_Pstr(PSTR("Es wurden schon "));
          debug_printInt(rf_tx_buffer.bytecount);
          debug_write_Pstr(PSTR(" bytes von "));
          debug_printInt(rf_tx_buffer.num_bytes);
          debug_write_Pstr(PSTR(" gesendet.\r\n"));
        }
      #endif
      //reset counter
    	rf_tx_buffer.bytecount = 0;
      // wait pseudo random ticks before sending again
      channel_free_count = 1000 + (time_ms % 500);
    }
  }

	//don't disturb RFM12 if transmitting
	if(rfm12_mode == MODE_TX) return;

  // check timeout of receiver (ms)
  // TODO: Zeit aus Baudrate und Pufferl�nge berechnen 
  if ((time_ms-rf_rx_lasttime)>100 && rf_rx_buffer.status != RFM12_STATUS_IDLE)
  {
    // Bei Timeout den Puffer wieder auf Idle zur�cksetzen
  	RFM12_INT_OFF();

    #ifdef RMF12_RAW_MODE
      // Im Raw-Mode bei Timeout Telegramm beenden
			//indicate that the Buffer is ready to be used
			rf_rx_buffer.rf_buffer_in->status = RFM12_STATUS_COMPLETE;
			//switch to other buffer
			uint8_t num = rf_rx_buffer.buffer_in_num;
    ...
    debug_write_str("Receive Timeout\r\n");
  }
  
  // don�t disturb receiving
  if (rf_rx_buffer.status != RFM12_STATUS_IDLE) return;
	...
	if(status & RFM12_STATUS_RSSI)
  {
    ...
    // pseudo random
		channel_free_count = 200 + (time_ms % 200);
	}
  else
  {
...
{
  if (len > RFM12_TX_BUFFER_SIZE)
  {
    debug_write_str ("length error\r\n");
    return RFM12_TX_ERROR;
  }
	//exit if the buffer isn't free
	if(rf_tx_buffer.status != RFM12_STATUS_FREE)
  {
    debug_write_str ("tx occupied\r\n");
		return RFM12_TX_OCCUPIED;
	}

  debug_write_str ("sending: ");
  #if RFM12_DEBUGLEVEL > 0
  ...
  memcpy ( rf_tx_buffer.buffer, data, len );
  	...
  	//2 sync bytes + HEADER + message length + 1 dummy byte
  	rf_tx_buffer.num_bytes = len + RFM12_HEADER_SIZE + 3;	
  	//write header to buffer
  	rf_tx_buffer.len = len;
  	rf_tx_buffer.type = type;
    rf_tx_buffer.crc_data = crc8(rf_tx_buffer.buffer,len);
  	rf_tx_buffer.crc_header = crc8(&rf_tx_buffer.len,RFM12_HEADER_SIZE-1);
	#else
    // RAW-Mode ohne Header
  	//2 sync bytes + message length + 1 dummy byte
  	rf_tx_buffer.num_bytes = len + 3;	
  #endif
	//reset counter
	rf_tx_buffer.bytecount = 0;
	
	//schedule packet for transmission
	rf_tx_buffer.status = RFM12_STATUS_OCCUPIED;
...
#else
  	RFM12_DDR_MOSI |= (1<<RFM12_BIT_MOSI);
    RFM12_DDR_SCK  |= (1<<RFM12_BIT_SCK);
    RFM12_DDR_SPI_SS |= (1<<RFM12_BIT_SPI_SS);
...
void rfm12_init()
{
  ...
  rfm12_mode = MODE_RX;
	...
	rfm12_data(RFM12_CMD_CFG | RFM12_CFG_EL | RFM12_CFG_EF | RFM12_BAND | RFM12_XTAL_12PF);
	...
	//set frequency
	rfm12_data(RFM12_CMD_FREQUENCY | RFM12_FREQUENCY_CMD );

	//set datarate
	rfm12_data(RFM12_CMD_DATARATE | RFM12_DATARATE_VALUE );

	//set PLL-Parameters
	rfm12_data(RFM12_CMD_PLL | RFM12_PLL_10MHZ );
