...
bool TMRegExp::SetExpression(string value, int maxmatch)
{
  if(FExpression != value)
  {
    ...
    if (maxmatch==0)
    {
      // Das ist eine ganz einfache Methode um die Komplexität
      // des regulären Ausdrucks abzuschätzen
      // Es werden auf jeden Fall mehr matches gebraucht als
      // Zeichenketten am Ende zurückgeliefert werden
    ...
    // TODO: regerror
    #ifndef _NO_EXEPTOBJ
    if (err) MEXCEPTION(0,"Fehler beim Erzeugen des regulären Ausdrucks: '"+FExpression+"'","");
    #endif
    #endif
    #ifdef STDREGEX
    try {
      ...
      #ifndef _NO_EXEPTOBJ
      MEXCEPTION(0,"Fehler beim Erzeugen des regulären Ausdrucks: '"+FExpression+"'",e.what());
      #endif
      compiled = false;
    }
    #endif
...
void TMRegExp::GetMatches(vector<string> &list)
{
  ...
  #endif
  #ifdef STDREGEX
  for (auto i=aRegMatch.begin(); i != aRegMatch.end(); i++) {
    list.push_back(*i);
  }
  #endif
}

string TMRegExp::GetMatch(int num, int &pos)
{
  if (!matched) return "";
  #ifdef PERLREGEX
  int n=0;
  for (int i = 1; i < maxmatch; i++)
  {
    if (aRegMatch[i].rm_so >= 0)
    {
      if (num==n)
      {
        ...
        {
          #ifndef _NO_EXEPTOBJ
          MEXCEPTION(0,"Fehler beim Ermitteln einer Zeichenkette im regulären Ausdruck '"
  ...
  if (aRegMatch.size()<num) {
    ...
    return aRegMatch[num];
  }
  #endif
  return "";
}

string TMRegExp::GetMatch(int num)
{
  int pos;
  return GetMatch(num,pos);
}
