...
void TMSocket::Start()
{
  ...
  if (RefCount==0)
  {
    ...
    if (rc!=0)
    {
      throw Exception("WinSock lässt sich nicht starten.");
    }
  }
  RefCount++;
  #endif
...
void TMSocket::Hostport2HostPort(std::string hostport, std::string &host, int &port)
{
  ...
  if (!rx.Exec(hostport))
  {
    throw Exception(("Fehler bei '"+hostport+"'. Host und Port müssen in der Form Hostname:Portnummer angegeben werden.").c_str());
  }
  host = rx.GetMatch(0);
...
std::string TMSocket::Host2Ip(std::string host)
{
  ...
  if (!rx.Exec(host))
  {
    // TODO: Namensauflösung mit gethostbyname
    throw Exception(("'"+host+"' muss eine gültige IP-V4-Adresse sein.").c_str());
  }
  return host;
}

std::wstring TMSocket::GetLastError()
{
  #if (_Windows)
  wchar_t *s = NULL;
